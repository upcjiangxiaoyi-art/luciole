...
(function () {
    ...
    var BUILTIN_PROMPTS = {
        ...
        steps: [
            ...
            '· 一段戏要「丰富」，靠的是节奏起落——日常、意外、困境、机会、余味。哪里该慢，哪里该出事，哪里该给他们一个能靠近（或翻脸）的机会？机会是事件，靠不靠近是他们的事。',
            '· 玩家点名要的那些事（采摘、写生、大雨……）各放在第几步最合适？她没写、但这段戏该有的，补上。',
            '',
            '然后切步。每一步是「这一轮世界里发生的一件事」。这里有一条铁律，比其他所有要求都重要：',
            '**只给事件，不给反应。** 步骤写的是世界做了什么、以及谁知道什么；user 和角色怎么反应、说什么、做什么、心里怎么想，一个字都不写——那是他们两个人的戏，你写了就等于替他们演了。',
            '反例（绝对不要）：「暴雨突至，两人抱在一起躲雨」——后半句是反应。「他发疯一样满城找她」——整句都是反应。',
            '正例：「山上突降暴雨，雨大到看不清路，最近的遮蔽是半山一座废弃的护林站」——只有事件，躲不躲、怎么躲，他们自己来。',
            '正例：「她带着孩子离开了这座城，在南方落脚；他至今不知道有孩子这件事」——事件加信息状态（谁知道、谁不知道），找不找她，不写。',
            '写法：',
            '· 一步只装一件事，装得实：地点、时间、天气、出现的东西或人、发生的变故、每个人此刻知道与不知道的。40～120 字。感官细节可以写，但写的是环境本身（雨声、泥味、光线），不是谁闻到了、谁觉得冷。',
    ...
    var PROMPT_SLOT_META = [
        ...
        { key: 'steps',     title: '剧情步骤提示词（第二幕 · 想玩什么→N 步）',
          vars: '（无）',
          owned: '「先【分析】再【步骤】、步数严格等于指定数」的格式由代码追加；贴耳注入的措辞（含「只给了事件，反应由你演」）写死在代码里。',
          rows: 9 },
    ...
    function panelHtml() {
        ...
        '          <div class="lcl2-dim">你说清楚想玩什么，小萤火<b>强制</b>把它想成 N 步；之后每一轮只把<b>一步</b>贴到演员耳边，后面的步骤它看不见——所以它没法一层就把山登完回家。步骤<b>只给事件，不给反应</b>：下雨了是事件，抱在一起是反应；反应留给你们两个演。想的过程会实时流到下面。</div>' +
        ...
        '            <textarea id="lcl2_step_text" class="text_pole" rows="3" placeholder="这一轮世界里发生的一件事。只给事件与谁知道什么，不写任何一方的反应、动作、台词"></textarea>' +
    ...
    var STEP_TRAILER = [
        '这是导演给这一回合的调度：本回合的正文必须走到上面这件事，让它在场景里实实在在地发生。',
        '上面只给了事件，没给反应。在场的人怎么反应、说什么、做什么，由你按各自的性格和此刻的处境来演，不要照着任何预期的走向演。',
    ...
    function renderStepPanel(force) {
        ...
        for (var i = 0; i < sb.steps.length; i++) {
            ...
            html += '<div class="lcl2-step' + (isCur ? ' lcl2-act-on' : '') + '" data-id="' + esc(s.id) + '">'
                ...
                + '<textarea class="lcl2-step-f text_pole" data-f="text" rows="3" placeholder="这一轮世界里发生的一件事（只给事件，不给反应）"' + (!trim(s.text) ? ' style="border-color:var(--lcl2-danger)"' : '') + '>' + esc(s.text) + '</textarea>'
                + '</div>';
        }
        $host.html(html);
    }
